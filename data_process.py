...
def norm_num(data_file, dataset_path):
    ...
    minNum = min(scf_data.shape[0], vps_data.shape[0], dns_data.shape[0],  dns_cc_data.shape[0]
                 , scf_cc_data.shape[0], vps_cc_data.shape[0])
    ...
    ben_data = ben_data.sample(6 * minNum + 2 * minNumDoH)
    ...
    # 数据合并
    train_data = pd.concat([ben_train_data, scf_train_data, vps_train_data, dns_train_data, doh_train_data,
                            scf_cc_train_data, vps_cc_train_data, dns_cc_train_data, doh_cc_train_data])
    test_data = pd.concat([ben_test_data, scf_test_data, vps_test_data, dns_test_data, doh_test_data,
                           scf_cc_test_data, vps_cc_test_data, dns_cc_test_data, doh_cc_test_data])
    # 二分类的
    ...
    y_train = np.array([0] * len(ben_train_data) + [1] * len(scf_train_data) + [2] * len(vps_train_data)
                       + [3] * len(dns_train_data) + [4] * len(doh_train_data) + [5] * len(scf_cc_train_data)
                       + [6] * len(vps_cc_train_data) + [7] * len(dns_cc_train_data) + [8] * len(doh_cc_train_data))
    y_test = np.array([0] * len(ben_test_data) + [1] * len(scf_test_data) + [2] * len(vps_test_data)
                      + [3] * len(dns_test_data) + [4] * len(doh_test_data) + [5] * len(scf_cc_test_data)
                      + [6] * len(vps_cc_test_data) + [7] * len(dns_cc_test_data) + [8] * len(doh_cc_test_data))
    ...
    print("--------------------")
    # print(test_data)
    np.save(dataset_path + '810train_data', train_data)
    np.save(dataset_path + '810test_data', test_data)


def norm_test(data_file, dataset_path):
    unnorm_cols = ['Src IP', 'Src Port', 'Dst IP', 'Dst Port', 'Label']
    ...
    vps_cc_data = vps_cc_data.sample(minNum)
    dns_cc_data = dns_cc_data.sample(minNum)
    ben_data = ben_data.sample(6 * minNum + doh_data.shape[0] + doh_cc_data.shape[0])

    test_data = pd.concat([ben_data, scf_data, vps_data, dns_data, doh_data,
                           scf_cc_data, vps_cc_data, dns_cc_data, doh_cc_data])

    print("合并后test_data.shape is ", test_data.shape)

    test_data = test_data.drop(columns=unnorm_cols)
    print("删除五元组后test_data.shape is ", test_data.shape)
    y_test = np.array([0] * len(ben_data) + [1] * len(scf_data) + [2] * len(vps_data)
                      + [3] * len(dns_data) + [4] * len(doh_data) + [5] * len(scf_cc_data)
                      + [6] * len(vps_cc_data) + [7] * len(dns_cc_data) + [8] * len(doh_cc_data))
    ...
    print("归一化后")
    print(test_data.shape)
    np.save(dataset_path + 'test_data1', test_data)
